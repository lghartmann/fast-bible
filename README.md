...
- The index name is hardcoded as `bible` in `app/main.go`.
- The Elasticsearch address is currently hardcoded as `http://localhost:9200` in `app/elasticsearch/elasticsearch.go`.
- Running the app again will skip index creation if the index already exists, but it will still attempt to index the documents again using the same IDs.

## Possible next improvements

- Add configuration through environment variables
- Add a small HTTP API for verse search
- Add bulk indexing for better performance
